...
function proxyRequest(req, res, proxy) {
    ...
    buffer: {
      pipe: function(proxyReq) {
        ...
        if (process.env.ENABLE_REQUEST_LOGGING === 'true') {
          // Log the request details to the target endpoint
          console.log('\n=== Proxy Request ===');
  ...
  /// Below headers are needed for requests to work with Onfinality's mx-api: https://doc.onfinality.io/
  /// TODO: Need to determine if overriding all these headers is absolutely needed
  // Override browser headers
  delete req.headers['content-type'];
  req.headers['content-type'] = 'application/json';
  req.headers['date'] = new Date(Date.now()).toUTCString();
  req.headers['origin'] = 'http://localhost/';
  delete req.headers['user-agent'];
  req.headers['User-Agent'] = 'PostmanRuntime/7.41.1';
  delete req.headers['accept'];
  req.headers['Accept'] = '*/*';
  delete req.headers['Cache-Control'];
  req.headers['Cache-Control'] = 'no-cache';
  req.headers['host'] = 'api.onfinality.io';
  ...
  // Delete custom x-date header that bypasses forbidden headers in the browser:
  // https://fetch.spec.whatwg.org/#forbidden-header-name
  delete req.headers['x-date'];
